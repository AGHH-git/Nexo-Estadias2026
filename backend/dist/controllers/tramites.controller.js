...
exports.descargarPDFPrefilled = exports.buscarEmpresas = exports.subirEvidencia = exports.actualizarTramite = exports.crearTramite = exports.obtenerTramiteAlumno = void 0;
const database_1 = require("../config/database");
const pdf_lib_1 = require("pdf-lib");
// 1. Obtener el trámite activo de un alumno
const obtenerTramiteAlumno = async (req, res) => {
    if (!req.usuario) {
    ...
    try {
        ...
        const resultMatricula = await database_1.pool.query(queryMatricula, [req.usuario.id]);
      ...
      LIMIT 1
    `;
        const resultTramite = await database_1.pool.query(queryTramite, [matricula]);
    ...
    `;
        const resultObs = await database_1.pool.query(queryObs, [tramite.id]);
...
exports.obtenerTramiteAlumno = obtenerTramiteAlumno;
...
const crearTramite = async (req, res) => {
    ...
    `;
        ...
        await client.query('INSERT INTO logs_auditoria (usuario_id, accion, detalles) VALUES ($1, $2, $3)', [req.usuario.id, 'CREAR_TRAMITE', `Alumno con matrícula ${matricula} registró trámite ID ${nuevoTramite.id}`]);
...
const actualizarTramite = async (req, res) => {
    ...
    `;
        ...
        // Registrar log
        await client.query('INSERT INTO logs_auditoria (usuario_id, accion, detalles) VALUES ($1, $2, $3)', [req.usuario.id, 'ACTUALIZAR_TRAMITE', `Alumno actualizó trámite ID ${tramiteId}`]);
        await client.query('COMMIT');
        return res.status(200).json({
...
const subirEvidencia = async (req, res) => {
    ...
    try {
        ...
        const resTramite = await database_1.pool.query(queryTramite, [tramiteId]);
        if (resTramite.rows.length === 0) {
            return res.status(404).json({ mensaje: 'Trámite no encontrado.' });
        }
        const tramite = resTramite.rows[0];
        // Validar que el alumno que sube sea el dueño
        const resAlumno = await database_1.pool.query('SELECT matricula FROM alumnos WHERE usuario_id = $1', [req.usuario.id]);
       ...
       WHERE id = $2`, [rutaEvidenciaRelativa, tramiteId]);
        // Auditoría
        await database_1.pool.query('INSERT INTO logs_auditoria (usuario_id, accion, detalles) VALUES ($1, $2, $3)', [req.usuario.id, 'SUBIR_EVIDENCIA', `Alumno subió evidencia firmada y completó trámite ID ${tramiteId}`]);
...
exports.subirEvidencia = subirEvidencia;
// 5. Buscar empresas
const buscarEmpresas = async (req, res) => {
    ...
    try {
      ...
      LIMIT 10
    `;
        const result = await database_1.pool.query(queryBuscar, [`%${queryTerm}%`]);
...
exports.buscarEmpresas = buscarEmpresas;
// 6. Generar formato FODVI08-H prellenado con pdf-lib
const descargarPDFPrefilled = async (req, res) => {
    if (!req.usuario) {
        return res.status(401).json({ mensaje: 'No autorizado.' });
    }
    const tramiteId = parseInt(req.params.id, 10);
    try {
      ...
      WHERE t.id = $1
    `;
        const result = await database_1.pool.query(queryDatos, [tramiteId]);
        ...
        const colorPrimario = (0, pdf_lib_1.rgb)(143 / 255, 12 / 255, 52 / 255); // Rojo vino
        const colorDorado = (0, pdf_lib_1.rgb)(200 / 255, 163 / 255, 120 / 255); // Dorado
        const colorGrisClaro = (0, pdf_lib_1.rgb)(240 / 255, 240 / 255, 240 / 255);
        // ENCABEZADO
        page.drawRectangle({
            x: 30,
            y: height - 60,
        ...
        linesProb.slice(0, 3).forEach((line) => {
        ...
        // Líneas para firmas
        // Firma Alumno
        page.drawLine({ start: { x: 40, y: currentY }, end: { x: 180, y: currentY }, thickness: 1, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
        ...
        page.drawLine({ start: { x: 230, y: currentY }, end: { x: 380, y: currentY }, thickness: 1, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
        ...
        page.drawLine({ start: { x: 430, y: currentY }, end: { x: 570, y: currentY }, thickness: 1, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
...
exports.descargarPDFPrefilled = descargarPDFPrefilled;
